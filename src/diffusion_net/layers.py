...
class MiniMLP(nn.Sequential):
    '''
    A simple MLP with configurable hidden layer sizes. nn.ReLU
    scape activation=nn.LeakyReLU
    faust activation=nn.PReLU
    '''
    def __init__(self, layer_sizes, dropout=False, activation=nn.PReLU, name="miniMLP"):
...
class WTFMNet(nn.Module):
    def __init__(self, C_in,  C_out, n_fmap, is_mwp=True):
    ...
    def forward(self,descs_x,massvec_x,evals_x,evecs_x,gs_x,gradX_x,gradY_x,\
        ...
        #最小二乘法求解init C
        feat_x_hat = torch.matmul(
            (evecs_x*massvec_x.unsqueeze(-1)).t(), feat_x)
        feat_y_hat = torch.matmul(
            (evecs_y*massvec_y.unsqueeze(-1)).t(), feat_y)
        
        Cyx_t=torch.linalg.lstsq(feat_y_hat.t(),feat_x_hat.t()).solution
        self.C=Cyx_t.t()
        ...
        self.feat_correspondences(evecs_x,evecs_y@self.C.transpose(-2,-1))
        loss=self.criterion(evecs_x,self.p@evecs_y@self.C.transpose(-2,-1))
...
class WTFMNet_orth(nn.Module):
    ...
    def forward(self,descs_x,massvec_x,evals_x,evecs_x,gs_x,gradX_x,gradY_x,\
        ...
        ky=self.C.size(1)
        Iy=torch.eye(ky,device=self.C.device)
        loss=self.criterion(self.C.transpose(-2,-1)@self.C,Iy)
...
class WTFMNet_MLP(nn.Module):
    def __init__(self, in_dim=352, n_fmap=50, is_mwp=True):
        super().__init__()
        
        self.n_fmap=n_fmap
        self.is_mwp=is_mwp
        self.feat_extrac=RefineNet(in_dim=in_dim)
        self.criterion=torch.nn.MSELoss()
    

    def forward(self,descs_x,massvec_x,evecs_x,gs_x,descs_y,massvec_y,evecs_y,gs_y):
        feat_x=self.feat_extrac(descs_x)
        feat_y=self.feat_extrac(descs_y)
